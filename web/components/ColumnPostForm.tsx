...
type ColumnCategory = "CRYPTO_NEWS" | "MEME_COIN";

type ColumnPostFormProps = {
...
export function ColumnPostForm({
  category,
  heading,
  intro,
  loginPrompt,
  ...
  if (status === "unauthenticated") {
    return <p className="text-sm text-zinc-500">{loginPrompt}</p>;
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="mx-auto flex max-w-xl flex-col gap-3">
      ...
      <label className="text-xs text-zinc-500">
        {quoteLabel}(最大{MAX_QUOTE_LENGTH}文字)
        <textarea
          required
      ...
      <label className="text-xs text-zinc-500">
        {commentaryLabel}(最低{MIN_COMMENTARY_LENGTH}文字)
        <textarea
          required
